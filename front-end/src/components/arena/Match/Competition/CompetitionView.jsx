...
export default function CompetitionView() {
  ...
  return (
    <div>
      ...
      <div className="competition-view max-h-[535px]">
        {/* 왼쪽(6)에 해당하는 부분 */}
        <div className="w-full left-panel ml-3 mr-3 mt-1" style={{ width: `${panelWidths.left}%`}}>
          <div
            className="h-[95%] rounded-xl shadow-lg p-4"
            style={{ backgroundColor: '#F5EBDB' }}
          >
        ...
        <div className="right-panel mr-3 relative" style={{ width: `${panelWidths.right}%`, display: 'flex', flexDirection: 'column' }}>
          ...
          {/* 입력 폼 */}
          <form onSubmit={handleSendMessage} className="flex pl-4 pt-10">
            <input
              type="text"
              className="w-full rounded-3xl border-2 border-gray-300 px-4 py-2 flex-grow focus:outline-none" 
            ...
            <button
              type="submit"
              className="w-[20%] bg-blue-300 text-white rounded-full px-4 py-2 ml-2 focus:outline-none"
            >
              입력
            </button>
          </form>
